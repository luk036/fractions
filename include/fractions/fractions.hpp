...
template <typename Z> struct Fraction {
     ...
     * @param[in] numer
     */
    CONSTEXPR14 explicit Fraction(Z &&numer)
        : _numer{std::move(numer)}, _denom(Z(1)) {}
    ...
    CONSTEXPR14 auto operator+(const Fraction &rhs) const -> Fraction {
        ...
        if (common == Z(0)) {
            return Fraction(
                rhs._denom * this->_numer + this->_denom * rhs._numer, Z(0));
        }
     ...
     * @return Fraction
     */
    friend CONSTEXPR14 auto operator+(int &&c, const Fraction &frac)
        -> Fraction {
        return frac + Z(c);
    }
